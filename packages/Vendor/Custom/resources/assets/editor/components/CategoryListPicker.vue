...
function onSelect(category: Category) {
  model.value = [...(model.value ?? []), category.id];
}

function onDeselect(category: Category) {
  model.value = (model.value ?? []).filter(id => id !== category.id);
}
    ...
    >
      <div class="border rounded">
        ...
        >
          ...
          >
            + {{ selectedCategories.length - 3 }} {{ t('more items') }}
          </li>
        </ul>
        <Dialog.Trigger
          class="flex items-center px-3 h-10 gap-4 w-full text-sm hover:bg-neutral-50"
          :class="{ 'border-t': selectedCategories.length }"
        >
          <span class="flex-1 text-left">
            {{ selectedCategories.length ? t('Change') : t('Select categories') }}
          </span>
      ...
      <Dialog.Positioner class="flex absolute z-50 inset-0 w-full h-full items-center justify-center">
        <Dialog.Content class="bg-white shadow flex flex-col w-full h-full overflow-hidden">
          <header class="flex-none h-12 border-b border-neutral-200 flex gap-3 px-4 items-center justify-between">
            <Dialog.Title>{{ field.label || t('Select categories') }}</Dialog.Title>
            <button
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  @click.prevent="onDeselect(category)"
                >
